...
- **Stay oriented:** Environment indicator uses a stronger color strip; Classic/Fluid/Nav mode badge shows where you are; first-install onboarding checklist in Options.
...
- **Bulk / repeated data entry:** Enable **Field Entry** (Options → Optional / careful). Use **Capture** on a filled page (including Fluid and same-origin popup/modal frames), then **Paste** on the next employee/guest/job row with eligibility preview. **From sheet** accepts TSV/CSV — one data row fills the page; **multiple data rows** map to grid `$0`/`$1`/… and click **Add Row** when more rows are needed. **Find / Replace** remaps current values. Save named profiles locally (export warns about business keys). Avoids fighting Chrome autocomplete.
...
P3 Wave 5 items (SP-01…SP-08) shipped in **1.0.13**. Further ideas stay in:
